...
export default function AssetChart() {
  ...
  const chartData = useMemo(() => {
    const total = data.assets.reduce((acc, item) => acc + item.valueInUSD, 0);
    const d: ChartData[] = [];
    data.assets.slice(0, 3).forEach((item, idx) => {
      d.push({
        name: item.name,
        y: item.valueInUSD,
    ...
    if (data.assets.length > 3) {
      const value = data.assets.slice(3).reduce((acc, item) => acc + item.valueInUSD, 0);
      d.push({
        name: 'Others',
        y: value,
          ...
          {chartData.map((item) => (
            <Grid key={item.name} item xs={6}>
              <Stack direction="row" spacing={1}>
                ...
                />
                <Box>
                  <Link href={item.entity ? getEntityUrl(item.entity) : '#'}>
