...
export default function ProtocolChart() {
  ...
  const chartData = useMemo(() => {
    const total = data.dapps.reduce((acc, item) => acc + item.value, 0);
    const d: ChartData[] = [];
    data.dapps.slice(0, 3).forEach((item, idx) => {
      d.push({
        name: item.name,
        y: item.value,
    ...
    if (data.assets.length > 3) {
      const value = data.dapps.slice(3).reduce((acc, item) => acc + item.value, 0);
      d.push({
        name: 'Others',
        y: value,
